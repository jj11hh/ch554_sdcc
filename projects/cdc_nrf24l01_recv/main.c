...
void GPIOSetup( void ) {
    ...
    PWM_CTRL = bPWM_IE_END | bPWM_IF_END | bPWM2_POLAR | bPWM2_OUT_EN;
    PWM_DATA2 = 0;
    PWM_CK_SE = 1;
    IE_PWMX = 1;
...
volatile uint16_t led_hl_count = 0;

void LEDInterrupt ( void ) __interrupt( INT_NO_PWMX ) {
    if (led_hl_count) {
        PWM_DATA2 = 255;
        led_hl_count --;
    }
...
void IRQ_ISR ( void ) __interrupt( INT_NO_INT0 ) {
    ...
    if (nRF24_GetStatus_RXFIFO() != nRF24_STATUS_RXFIFO_EMPTY){
        nRF24_CE_L();
        nRF24_ReadMBReg(nRF24_CMD_R_RX_PAYLOAD, rcv_buf, 22);
        nRF24_ClearIRQFlags();
        nRF24_CE_H();
    }
}
