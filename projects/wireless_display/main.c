...
volatile bool ping = 0;

// Handle IRQ pin
void IRQ_ISR ( void ) __interrupt( INT_NO_INT0 ) {
    if (nRF24_GetStatus_RXFIFO() != nRF24_STATUS_RXFIFO_EMPTY){
        nRF24_CE_L();

        uint8_t buf;

        nRF24_ReadPayload(&buf, 1);
        nRF24_ClearIRQFlags();
        nRF24_CE_H();
...
void UsbCDC_Write(const char *buf, uint8_t len) {
    // write upload endpoint
    while (!UsbConfig || UpPoint2_Busy);
...
#define UsbCDC_CountRX USBByteCount

uint8_t UsbCDC_Read(char *buf, uint8_t max) {
    uint8_t count = 0;
    if (max == 0) return 0;
    while (!UsbConfig || !UsbCDC_CountRX);
    while (UsbCDC_CountRX && max) {
        *buf = Ep2Buffer[USBBufOutPoint ++];
        buf ++;
        max --;
        UsbCDC_CountRX --;
        count ++;
...
void UsbCDC_ReadUntil(char *buf, uint8_t until) {
    while (!UsbConfig || !UsbCDC_CountRX);
    while (until){
        if (UsbCDC_CountRX) {
            *buf = Ep2Buffer[USBBufOutPoint ++];
...
static inline void nRF24_Setup () {
    nRF24_Init();
    ...
    nRF24_WriteReg(nRF24_REG_RF_CH, 120);
    nRF24_WriteReg(nRF24_REG_RF_SETUP, 0b00001100);

    //nRF24_SetAddr(nRF24_REG_RX_ADDR_P0, (uint8_t *)"ziyan");
    //nRF24_SetAddr(nRF24_REG_TX_ADDR, (uint8_t *)"ziyan");

    nRF24_CSN_L();
    nRF24_LL_RW(nRF24_CMD_W_REGISTER | nRF24_REG_RX_ADDR_P0);
    nRF24_LL_RW('z');
    nRF24_LL_RW('i');
    nRF24_LL_RW('y');
    nRF24_LL_RW('a');
    ...
    // 250us, 4 times retry
    nRF24_SetAutoRetr(nRF24_ARD_250us, 16);


    // 接收数据长度 17 bytes
    // 行号 + 行数 (128 bit == 16 Byte)
    nRF24_WriteReg(nRF24_REG_RX_PW_P0, 1);

    //nRF24_WriteReg(nRF24_REG_CONFIG, 0x0E); //TX
    nRF24_WriteReg(nRF24_REG_CONFIG, 0x0F); //RX
...
bool send_block(uint8_t *block) {
    ...
    while (1) {
        ...
        if (status & ( nRF24_FLAG_MAX_RT | nRF24_FLAG_TX_DS )){
            nRF24_ClearIRQFlags();
            nRF24_FlushTX();
            break;
        }
...
void main( void )
{
    ...
    while(1)
    {
        __xdata uint8_t buf[17];
        uint8_t reply;
        uint8_t nread = 0;
        while (nread < 17) {
            nread += UsbCDC_Read(buf + nread, 17 - nread);
        }
        LED = 1; 
        if (send_block(buf))
            reply = 'S';
